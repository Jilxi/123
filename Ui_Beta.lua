...
function library.new(library, name, theme)
	...
	function window.Tab(window, name, icon)
		...
		function tab.Section(tab, name, TabVal)
			...
			-- ========== 轮盘菜单 ==========
function section.Wheel(section, text, options, callback)
    local callback = callback or function() end
    local options = options or {}
    assert(text, "No text provided")

    local WheelModule = Instance.new("Frame")
    local WheelBack = Instance.new("TextButton")
    local WheelBackC = Instance.new("UICorner")
    local WheelOpenBtn = Instance.new("TextButton")

    WheelModule.Name = "WheelModule"
    WheelModule.Parent = Objs
    WheelModule.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    WheelModule.BackgroundTransparency = 1.000
    WheelModule.BorderSizePixel = 0
    WheelModule.Size = UDim2.new(0, 428, 0, 38)

    WheelBack.Name = "WheelBack"
    WheelBack.Parent = WheelModule
    WheelBack.BackgroundColor3 = zyColor
    WheelBack.BackgroundTransparency = zyColorTransparency
    WheelBack.BorderSizePixel = 0
    WheelBack.Size = UDim2.new(0, 428, 0, 38)
    WheelBack.AutoButtonColor = false
    WheelBack.Font = Enum.Font.GothamBold
    WheelBack.Text = "   " .. text
    WheelBack.TextColor3 = TextColor
    WheelBack.TextSize = 16.000
    WheelBack.TextXAlignment = Enum.TextXAlignment.Left

    WheelBackC.CornerRadius = UDim.new(0, 6)
    WheelBackC.Name = "WheelBackC"
    WheelBackC.Parent = WheelBack

    WheelOpenBtn.Name = "WheelOpenBtn"
    WheelOpenBtn.Parent = WheelBack
    WheelOpenBtn.AnchorPoint = Vector2.new(1, 0.5)
    WheelOpenBtn.BackgroundColor3 = Background
    WheelOpenBtn.BackgroundTransparency = BackgroundTransparency
    WheelOpenBtn.Position = UDim2.new(1, -6, 0.5, 0)
    WheelOpenBtn.Size = UDim2.new(0, 70, 0, 26)
    WheelOpenBtn.AutoButtonColor = false
    WheelOpenBtn.Font = Enum.Font.GothamBold
    WheelOpenBtn.Text = "打开轮盘"
    WheelOpenBtn.TextColor3 = TextColor
    WheelOpenBtn.TextSize = 12
    Instance.new("UICorner", WheelOpenBtn).CornerRadius = UDim.new(0, 6)
    ...
    local function createWheel()
        if wheelGui then return end

        wheelGui = Instance.new("ScreenGui")
        wheelGui.Name = "WheelMenu_" .. text
        ...
        backdrop.MouseButton1Click:Connect(closeWheel)
        ...
        wheelFrame.Size = UDim2.new(0, 320, 0, 320)
        wheelFrame.BackgroundColor3 = zyColor
        wheelFrame.BackgroundTransparency = zyColorTransparency
        Instance.new("UICorner", wheelFrame).CornerRadius = UDim.new(1, 0)

        local centerLabel = Instance.new("TextLabel")
        centerLabel.Parent = wheelFrame
        centerLabel.AnchorPoint = Vector2.new(0.5, 0.5)
        centerLabel.Position = UDim2.new(0.5, 0, 0.5, 0)
        centerLabel.Size = UDim2.new(0, 100, 0, 40)
        centerLabel.BackgroundTransparency = 1
        centerLabel.Text = text
        centerLabel.Font = Enum.Font.GothamBold
        centerLabel.TextColor3 = TextColor
        ...
        for i, opt in ipairs(options) do
            local angle = (i - 1) * (360 / count) - 90 -- 从正上方开始顺时针排列
            local rad = math.rad(angle)
            local x = math.cos(rad) * radius
            local y = math.sin(rad) * radius

            local itemBtn = Instance.new("TextButton")
            itemBtn.Name = "WheelItem_" .. tostring(i)
            itemBtn.Parent = wheelFrame
            itemBtn.AnchorPoint = Vector2.new(0.5, 0.5)
            itemBtn.Position = UDim2.new(0.5, x, 0.5, y)
            itemBtn.Size = UDim2.new(0, 64, 0, 64)
            itemBtn.BackgroundColor3 = Background
            itemBtn.BackgroundTransparency = BackgroundTransparency
            ...
            itemBtn.MouseButton1Click:Connect(function()
                spawn(function() Ripple(itemBtn) end)
                callback(opt)
                closeWheel()
            end)

            itemBtn.MouseEnter:Connect(function()
                Tween(itemBtn, { 0.15, "Linear", "InOut" }, { BackgroundTransparency = 0 })
            end)
            itemBtn.MouseLeave:Connect(function()
                Tween(itemBtn, { 0.15, "Linear", "InOut" }, { BackgroundTransparency = BackgroundTransparency })
            end)
        end
    ...
    WheelOpenBtn.MouseButton1Click:Connect(function()
        if isOpen then
            closeWheel()
        else
            createWheel()
